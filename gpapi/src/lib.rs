...
use googleplay_protobuf::{
    AcceptTosResponse, AndroidBuildProto, AndroidCheckinProto, AndroidCheckinRequest, AndroidCheckinResponse, BulkDetailsRequest, BulkDetailsResponse, DetailsResponse, DeviceConfigurationProto, DeviceFeature, ResponseWrapper, UploadDeviceConfigRequest, UploadDeviceConfigResponse
};
...
impl Gpapi {
    ...
    pub fn with_extra_device_properties<S: Into<String>>(device_codename: S, email: S, extra_device_properties_path: &Path) -> Self { 

        let _device_properties = read_device_properties_from_file(extra_device_properties_path, device_codename);
...
fn read_device_properties_from_file<S: Into<String>>(device_properties_path: &Path, device_codename: S) -> DeviceProperties {
    let mut config = Ini::new();
    config
        .read(fs::read_to_string(device_properties_path).expect("Can't read custom device.properties file."))
        .expect("Can't parse custom device.properties file.");
    ...
    mod gpapi {
        ...
        async fn test_download() {
            if let (Ok(email), Ok(aas_token)) = (env::var("EMAIL"), env::var("AAS_TOKEN")) {
                ...
                api.set_aas_token(aas_token);
                if api.login().await.is_ok() {
                    assert!(api.download("com.instagram.android", None, true, true, true, &Path::new("/tmp/testing"), None).await.is_ok());
                }
            }
        }
